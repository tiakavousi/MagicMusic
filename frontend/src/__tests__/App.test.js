...
describe('App Component', () => {
  ...
  test('fetches and displays musics', async () => {
    mock
      .onGet('http://localhost:5000/musics')
      .reply(200, [{ id: 1, name: 'Song1', singer: 'Singer1' }]);
  ...
  test('adds a new music', async () => {
    ...
    mock
      .onPost('http://localhost:5000/musics')
      .reply(201, { id: 1, name: 'Song1', singer: 'Singer1' });

    render(<App />);

    fireEvent.change(screen.getByPlaceholderText(/Music's Name/i), {
      target: { value: 'Song1' },
    });
    fireEvent.change(screen.getByPlaceholderText(/Singer's Name/i), {
      target: { value: 'Singer1' },
    });
